...
//extern NTSTATUS
//hdbktrap(IN PKTRAP_FRAME TrapFrame, IN PKEXCEPTION_FRAME ExceptionFrame, IN PEXCEPTION_RECORD ExceptionRecord, IN PCONTEXT ContextRecord, IN KPROCESSOR_MODE PreviousMode, IN BOOLEAN SecondChanceException);
//这里做一个跳转
//VOID
//ModifyKdpTrap(PVOID myaddress, PVOID targetaddress)
//{
//    KIRQL     irql;
//    ULONGLONG myfun;
...
HideDriver()
{
    ...
    while ((PKLDR_DATA_TABLE_ENTRY)entry->InLoadOrderLinks.Flink != firstentry)
    {
        ...
        {
            RtlUnicodeStringToAnsiString(&FileNameA, &(entry->FullDllName), TRUE);
            LogDebugInfo("HideDriver search = %s", FileNameA.Buffer);
...
PMDL (*IoAllocateMdOrig)(__drv_aliasesMem PVOID VirtualAddress, ULONG Length, BOOLEAN SecondaryBuffer, BOOLEAN ChargeQuota, PIRP Irp);
//这里做一个跳转
VOID
ModifyIoAllocateMdl(PVOID myaddress, PVOID targetaddress)
{
    KIRQL     irql;
    ULONGLONG myfun;
    UCHAR     jmp_code[] = "\x48\xB8\xFF\xFF\xFF\xFF\xFF\xFF\xFF\x00\xFF\xE0\x00\x00"; //mov rax xxx,jmp rax
...
PMDL
newIoAllocateMdl(__drv_aliasesMem PVOID VirtualAddress, ULONG Length, BOOLEAN SecondaryBuffer, BOOLEAN ChargeQuota, PIRP Irp)
{
    //debg();
    ...
    return IoAllocateMdOrig(VirtualAddress, Length, SecondaryBuffer, ChargeQuota, Irp);
}
//这里做一个还原
void
UnHookIoAllocateMdl()
...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------
VOID
GoTPUnInit(PDRIVER_OBJECT DriverObject)
{
    //还原之前的KdpTraphook
    //UnHookKdpTrap();
    //还原之前的IoAllocateMdl
